...
:deciduous_tree:类型检查器会查看`printLabel`的调用。 `printLabel`有一个参数，并要求这个对象参数有一个名为`label`类型为`string`的属性。 

需要注意的是，我们传入的对象参数实际上会包含很多属性，但是编译器只会检查那些必需的属性是否存在，并且其类型是否匹配。 然而，有些时候TypeScript却并不会这么宽松，我们下面会稍做讲解。

:deciduous_tree:下面我们重写上面的例子，这次使用接口来描述：必须包含一个`label`属性且类型为`string`：
...
:question:**需要注意的是，我们在这里并不能像在其它语言里一样，说传给 `printLabel`的对象实现了这个接口。**
...
:imp:带有可选属性的接口与普通的接口定义差不多，只是在可选属性名字定义的后面加一个`?`符号。
...
:smiling_imp: 你可以在属性名前用 `readonly`来指定只读属性:
...
上面代码的最后一行，可以看到就算把整个`ReadonlyArray`赋值到一个普通数组也是不可以的。 :ice_cream:但是你可以用**类型断言重写**：
...
**:2nd_place_medal:接口中未定义的属性在TS中会进行检查，并且会报错，如果不想要这些报错的信息，可以进行的操作：**
...
2. 然而，**最佳的方式是能够添加一个字符串索引签名**，前提是你能够确定这个对象可能具有某些做为特殊用途使用的额外属性。 如果 `SquareConfig`带有上面定义的类型的`color`和`width`属性，并且*还会*带有任意数量的其它属性，那么我们可以这样定义它：
...
3. 还有最后一种跳过这些检查的方式，这可能会让你感到惊讶，**它就是将这个对象赋值给一个另一个变量**： 因为 `squareOptions`不会经过额外属性检查，所以编译器不会报错。
...
:ab:**相当于将对象形式的接口定义为函数类型，直接应用的函数的类型判断，其中包括传入参数和返回值类型，如果新定义的函数没有写类型，ts会自行进行判断**
...
:question:不知道可不可以使用接口的函数定义的参数类型和原本的函数不一样呢？我觉得不行，试了之后确实不行==会报错，想想也是，怎么会可以呢哈哈哈
...
与C#或Java里接口的基本作用一样，`TypeScript`也能够用它来明确的强制一个类去符合某种契约。
...
因此，我们应该直接操作类的静态部分。 看下面的例子，我们定义了两个接口， `ClockConstructor`为构造函数所用和`ClockInterface`为实例方法所用。

 为了方便我们定义一个构造函数 `createClock`，它用传入的类型创建实例。

```ts
interface ClockConstructor {
    new (hour: number, minute: number): ClockInterface;//如果不在new后面写类型的话，就会报没有返回值的错误，ClockInterface写上就不会，那么如果不是对应的呢？我其实觉得可以，意思就是一个constructor对应多个实例部分==（有机会验证一波）
}
interface ClockInterface {
  tick(): void;
}
...
function createClock(
  ...
  return new ctor(hour, minute);//返回类型clockInterface一致
}
...
square.color = "blue"; //如果改为square.color = 1 就会报错，说明继承成功了
square.sideLength = 10;Try
```
...
function getCounter(): Counter {
  ...
  return counter; //返回值符合Counter 改为1则会报错
}
